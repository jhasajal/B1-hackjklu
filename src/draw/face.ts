...
function drawLabels(
  ...
  if (f.rotation?.angle?.yaw)
    l = replace(l, "[yaw]", rad2deg(f.rotation.angle.yaw));
  if (f.rotation?.angle?.pitch)
...
function drawIrisElipse(
  f: FaceResult,
  ctx: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D
) {
  // iris: array[center, left, top, right, bottom]
  if (f.annotations?.leftEyeIris && f.annotations?.leftEyeIris[0]) {
    ctx.strokeStyle = localOptions.useDepth
      ? "rgba(255, 200, 255, 0.3)"
      : localOptions.color;
    ctx.beginPath();
    const sizeX =
      Math.abs(
        f.annotations.leftEyeIris[3][0] - f.annotations.leftEyeIris[1][0]
      ) / 2;
    const sizeY =
      Math.abs(
        f.annotations.leftEyeIris[4][1] - f.annotations.leftEyeIris[2][1]
      ) / 2;
    ctx.ellipse(
      f.annotations.leftEyeIris[0][0],
      f.annotations.leftEyeIris[0][1],
      sizeX,
      sizeY,
      0,
      0,
    ...
    if (localOptions.fillPolygons) {
      ctx.fillStyle = localOptions.useDepth
  ...
  if (f.annotations?.rightEyeIris && f.annotations?.rightEyeIris[0]) {
    ...
    const sizeX =
      Math.abs(
        f.annotations.rightEyeIris[3][0] - f.annotations.rightEyeIris[1][0]
      ) / 2;
    const sizeY =
      Math.abs(
        f.annotations.rightEyeIris[4][1] - f.annotations.rightEyeIris[2][1]
      ) / 2;
    ctx.ellipse(
      f.annotations.rightEyeIris[0][0],
      f.annotations.rightEyeIris[0][1],
      sizeX,
      sizeY,
      0,
      0,
...
function drawGazeSpheres(
  f: FaceResult,
  ctx: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D
) {
  if (
    localOptions.drawGaze &&
    f.rotation?.angle &&
    typeof Path2D !== "undefined"
  ) {
    ctx.strokeStyle = "pink";
    const valX =
      f.box[0] + f.box[2] / 2 - (f.box[3] * rad2deg(f.rotation.angle.yaw)) / 90;
    const valY =
      f.box[1] +
      f.box[3] / 2 +
      (f.box[2] * rad2deg(f.rotation.angle.pitch)) / 90;
...
function drawGazeArrows(
  f: FaceResult,
  ctx: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D
) {
  if (
    localOptions.drawGaze &&
    f.rotation?.gaze.strength &&
    f.rotation.gaze.bearing &&
    f.annotations.leftEyeIris &&
    f.annotations.rightEyeIris &&
    f.annotations.leftEyeIris[0] &&
    f.annotations.rightEyeIris[0]
  ) {
    ctx.strokeStyle = "pink";
    ctx.fillStyle = "pink";
    const leftGaze = [
      f.annotations.leftEyeIris[0][0] +
        Math.sin(f.rotation.gaze.bearing) * f.rotation.gaze.strength * f.box[3],
      f.annotations.leftEyeIris[0][1] +
        Math.cos(f.rotation.gaze.bearing) * f.rotation.gaze.strength * f.box[2],
    ];
    arrow(
      ctx,
      [f.annotations.leftEyeIris[0][0], f.annotations.leftEyeIris[0][1]],
      [leftGaze[0], leftGaze[1]],
      4
    );
    const rightGaze = [
      f.annotations.rightEyeIris[0][0] +
    ...
    arrow(
      ctx,
      [f.annotations.rightEyeIris[0][0], f.annotations.rightEyeIris[0][1]],
      [rightGaze[0], rightGaze[1]],
      4
    );
  }
}

function drawFacePolygons(
  f: FaceResult,
  ctx: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D
) {
    ...
    if (f?.mesh.length >= 468) {
      for (let i = 0; i < f.mesh.length; i++) {
        ...
        if (localOptions.drawAttention) {
          if (facemeshConstants.LANDMARKS_REFINEMENT_LIPS_CONFIG.includes(i))
            point(
              ctx,
              f.mesh[i][0],
              f.mesh[i][1],
              (f.mesh[i][2] as number) + 127,
              localOptions
            );
          if (
            facemeshConstants.LANDMARKS_REFINEMENT_LEFT_EYE_CONFIG.includes(i)
          )
            point(
              ctx,
              f.mesh[i][0],
          ...
          if (
            facemeshConstants.LANDMARKS_REFINEMENT_RIGHT_EYE_CONFIG.includes(i)
          )
            point(
              ctx,
              f.mesh[i][0],
