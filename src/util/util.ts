...
export function log(...msg): void {
  ...
  const ts = `${dt.getHours().toString().padStart(2, "0")}:${dt
    ...
    .toString()
    .padStart(3, "0")}`;
  if (msg) console.log(ts, ""); // eslint-disable-line no-console
}
...
export function validate(
  ...
  parent = "config",
  msgs: { reason: string; where: string; expected?: string }[] = []
) {
  ...
  if (config.debug && parent === "config" && msgs.length > 0)
...
export const minmax = (data: number[]) =>
  data.reduce((acc: number[], val) => {
